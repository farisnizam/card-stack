...
const Deck = ({ cardList = defaultCards, verticalDrag = false, horizontalDrag = true }: DeckProps) => {
  ...
  const bind = useDrag(({ args: [index], down, movement: [mx, my], direction: [xDir, yDir], velocity }) => {
    ...
    api.start(i => {
      ...
      const x = horizontalDrag ? (isGone ? (200 + window.innerWidth) * dirX : down ? mx : 0) : 0 // When a card is gone it flys out left or right, otherwise goes back to zero
      const y = verticalDrag ? (isGone ? (200 + window.innerHeight) * dirY : down ? my : 0) : 0 // When a card is gone it flys out up or down, otherwise goes back to zero
      //   const y = 0 // When a card is gone it flys out up or down, otherwise goes back to zero
      const rot = mx / 100 + my / 100 + (isGone ? (dirX * 10 + dirY * 10) * velocity : 0) // How much the card tilts, flicking it harder makes it rotate faster
